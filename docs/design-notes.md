...
| プロトコルの後方互換ポリシー | [versioning.md](./versioning.md) に暫定案。**4 週間の API ロック計測が始まるまで**確定しない（下記） |

### 「API ロック開始」の意味

この行は以前ただ「API ロック開始まで」と書いており、二通りに読めた。**後者が正しい。**

1. **API ロック *ツール* の導入** —— 済んでいる。`pnpm api:check` が `pnpm verify` の中で走り、
   `api-lock.md` が実際の公開 API と食い違えば CI が落ちる。16 リポジトリすべてで有効
2. **4 週間の API ロック *計測* の開始** —— まだ。plan.md §6 Step 3 は
   「界面が安定した(API ロック 4 週間無変更)リポジトリから」publish に切り替えると定めており、
   その 4 週間はまだ 1 日も経っていない

後方互換ポリシーが待っているのは 2 のほうである。理由は、**publish されていないプロトコルに
後方互換は存在しない**からで、ツールが入ったかどうかとは関係がない。消費者が固定バージョンで
このパッケージを参照し始めて初めて「壊さない」という約束に意味が生まれる。

計測の起点は `api-lock.md` を最後に変更したコミットである。
